...
- **`src/css/quasar.variables.scss` — намеренная заглушка**, не забытый
  дубликат. `@quasar/app-vite` проверяет наличие файла переменных SCSS
  жёстко от корня проекта (`src/css/quasar.variables.${ext}`,
  `lib/cache/module.cssVariables.js`), в обход алиаса `@`, а затем
  загружает найденный файл уже через алиас (`@/css/...` → `src/web/css/`
  при нашей ремапе). Конфиг-опции подсунуть путь напрямую нет —
  `metaConf.css` целиком строится из этой автодетекции. Настоящий файл
  живёт в `src/web/css/quasar.variables.scss`; на верхнем уровне — только
  `@forward` на него. Без заглушки `variablesFile` тихо становится
  `false`: сборка проходит без ошибок, но переопределения Quasar-тем
  (шрифт, цвета) перестают применяться — тот же класс ловушки, что и с
  `sourceFiles.store`/Pinia выше. Не удалять `src/css/` и не пытаться
  «упростить» без отдельного решения этой проблемы в самом Quasar.
- **Файлы внутри `.githuman/` префиксуются** (`ght-reviews.db` по
  умолчанию, `resolveDbPrefix()`/`DEFAULT_DB_PREFIX` в `src/cli/config.ts`)
  — оригинальный [mcollina/githuman](https://github.com/mcollina/githuman)
  хранит свои данные в том же `<repo>/.githuman/reviews.db` (несовместимая
  схема), поэтому без префикса оба инструмента в одном репозитории затирали
  бы файлы друг друга. Префикс настраивается флагом `--db-prefix`
  (`serve`/`list`/`export` — указывать один и тот же) или переменной
